...
module.exports = new ButtonInterface({
    ...
    execute: async function(p){
        ...
        let exited = new p.embed().setDescription("You exited the application for TikTok.").setColor("SUCCESS")
        ...
        if(data?.promotion?.tiktok){            
            if(!data.promotion.tiktok.confirmed){
                data.promotion.tiktok.status = "In Progress"
                await data.save()
                ...
                .addFields(
                    {
                        name: "Link", value: `${data.promotion.tiktok.link||"None"}`
                    },
                    {
                        name: "Status", value: `${data.promotion.tiktok.status||"None"}`
                    }
                )
                .setColor("OBTAINED")
                ...
                data.promotion.tiktok.status = "Sending"
                await data.save()
            ...
            }else if(data.promotion.tiktok.confirmed){
                ...
                if(data.promotion.tiktok.status == "Waiting Approval"){
                ...
                }else if(data.promotion.tiktok.status == "Approved"){
                ...
                data.promotion.tiktok.status = data.promotion.tiktok.status == "Approved" ? "Approved" : "In Progress"
                await data.save()
                ...
                if(!proceed||proceed == 2){
                    data.promotion.tiktok.status = data.promotion.tiktok.status == "Approved" ? "Approved" : "Waiting Approval"
                    await data.save()
                    clear(p, 99)
            ...
            data = await p.mongo.queryOne("application", p.interaction.user.id, { _id: p.interaction.user.id, promotion: { _id: p.interaction.user.id, tiktok: { link: "", status: "In Progress", confirmed: false } } })
            try{
                if(!data?.promotion) data.promotion = { tiktok: { link: "", status: "In Progress", confirmed: false } }
                else if(!data.promotion.tiktok) data.promotion.tiktok = { link: "", status: "In Progress", confirmed: false }
                ...
                data.promotion.tiktok.link = link
                data.promotion.tiktok.status = "Unconfirmed"

                application.setDescription("Please press `Confirm` to confirm this is correct. If not please press `Deny`.")
                .addFields(
                    {
                        name: "Link", value: `${data.promotion.tiktok.link}`
                    },
                    {
                        name: "Status", value: `${data.promotion.tiktok.status}`
                    }
                )
                .setColor("OBTAINED")
...
function collectLink(p){
    return new Promise((resolve) => {
        ...
        collector.on('collect', (collected) => {
                let regexHandler = /(https?:\/\/)(www\.)?tiktok\.com\/@([\w-]+)/g
                let match = collected.content.match(regexHandler)
...
async function deleteApplication(p, data){
    ...
    if(!data?.promotion?.youtube&&!data?.promotion?.twitch){
        if(!data.nickname){
        ...
        await applicationSchema.updateOne({_id: p.interaction.user.id}, { $unset: { "promotion.tiktok": {} } })
        await data.save()
    }
}