...
module.exports = new ButtonInterface({
    ...
    execute: async function(p){
        ...
        let exited = new p.embed().setDescription("You exited the application for Twitch.").setColor("SUCCESS")
        ...
        if(data?.promotion?.twitch){            
            if(!data.promotion.twitch.confirmed){
                data.promotion.twitch.status = "In Progress"
                await data.save()
                ...
                .addFields(
                    {
                        name: "Link", value: `${data.promotion.twitch.link||"None"}`
                    },
                    {
                        name: "Status", value: `${data.promotion.twitch.status||"None"}`
                    }
                )
                .setColor("OBTAINED")
                ...
                data.promotion.twitch.status = "Sending"
                await data.save()
            ...
            }else if(data.promotion.twitch.confirmed){
                ...
                if(data.promotion.twitch.status == "Waiting Approval"){
                ...
                }else if(data.promotion.twitch.status == "Approved"){
                ...
                data.promotion.twitch.status = data.promotion.twitch.status == "Approved" ? "Approved" : "In Progress"
                await data.save()
                ...
                if(!proceed||proceed == 2){
                    data.promotion.twitch.status = data.promotion.twitch.status == "Approved" ? "Approved" : "Waiting Approval"
                    await data.save()
                    clear(p, 99)
            ...
            try{
                ...
                if(!proceed||proceed == 2) return p.interaction.editReply({embeds: [notResended], components: [], ephemeral: true})
                    ...
                    data.promotion.twitch.link = link
                    await data.save()
            ...
            data = await p.mongo.queryOne("application", p.interaction.user.id, { _id: p.interaction.user.id, promotion: { _id: p.interaction.user.id, twitch: { link: "", status: "In Progress", confirmed: false } } })
            try{
                if(!data?.promotion) data.promotion = { twitch: { link: "", status: "In Progress", confirmed: false } }
                else if(!data.promotion.twitch) data.promotion.twitch = { link: "", status: "In Progress", confirmed: false }
                ...
                data.promotion.twitch.link = link
                data.promotion.twitch.status = "Unconfirmed"

                application.setDescription("Please press `Confirm` to confirm this is correct. If not please press `Deny`.")
                .addFields(
                    {
                        name: "Link", value: `${data.promotion.twitch.link}`
                    },
                    {
                        name: "Status", value: `${data.promotion.twitch.status}`
                    }
                )
                .setColor("OBTAINED")
...
function collectLink(p){
    return new Promise((resolve) => {
        ...
        collector.on('collect', (collected) => {
                let regexPrefix = /(https?:\/\/)(www\.)?twitch\.(com|tv)\/(channel|user|c)/g
                let match = collected.content.match(regexPrefix)

                let regexHandler = /(https?:\/\/)(www\.)?twitch\.(com|tv)\/([\w-]+)/g
                let match2 = collected.content.match(regexHandler)
...
async function deleteApplication(p, data){
    ...
    if(!data?.promotion?.youtube&&!data?.promotion?.tiktok){
        if(!data.nickname){
        ...
        await applicationSchema.updateOne({_id: p.interaction.user.id}, { $unset: { "promotion.twitch": {} } })
        await data.save()
    }
}