...
module.exports = new ButtonInterface({
    ...
    execute: async function(p){
        ...
        let instructions = new p.embed().setDescription("**Please select the platform that you are promoting yourself on and provide a link to your channel/profile. \nFor example: YouTube -> https://youtube.com/TeamGosu\n__Only links are accepted no files.__ \n\n").setColor("OBTAIN")
        let timeUp = new p.embed().setDescription("I did not receive a link. Please try again in 1-2 minutes.").setColor("ERROR")
        ...
        let youtube = new ButtonBuilder().setLabel("YouTube").setStyle(2)
        let actionRow = new ActionRowBuilder().setComponents(youtube)

        let reply = p.interaction.reply({embeds: [instructions], components: [actionRow], ephemeral: true}).then(reply => {
            setTimeout(async ()=>{
